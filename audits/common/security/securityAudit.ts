...
class LoadAudit extends Audit {
  static get meta() {
    return {
      id: "common-security",
      title: "CERTIFICATO HTTPS - Il sito scuola deve avere un certificato https valido e attivo.",
      failureTitle: "CERTIFICATO HTTPS - Il sito scuola deve avere un certificato https valido e attivo.",
  ...
  static async audit(artifacts: LH.Artifacts & { origin: string }) : Promise<{ score: number; details: LH.Audit.Details.Table }> {
    ...
    const headings = [
      ...
      { key: "protocol", itemType: "text", text: "Protocollo usato dal dominio" },
      { key: "certificate_validation_from", itemType: "text", text: "Certificato valido da" },
      { key: "certificate_validation_to", itemType: "text", text: "Certificato valido a" },
...
async function checkCertificateValidation(url: string) : Promise<{ valid: boolean, valid_from: string, valid_to: string}>  {
  let returnObj = {
    valid: false,
    valid_from: "",
    valid_to: ""
...
async function checkTLSVersion(url: string) : Promise<{ valid: boolean, tls_version: string }> {
  let returnObj = {
    valid: false,
    tls_version: ""
  }
...
async function checkCipherSuite(url: string) : Promise<{ valid: boolean, version: string}> {
  let returnObj = {
    valid: false,
    version: ""
  }
  ...
  if (!Boolean(cipherInfo) || !Boolean(cipherInfo.version) || !Boolean(cipherInfo.standardName)) {
     return returnObj
  }
